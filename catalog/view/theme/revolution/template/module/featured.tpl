				...
				<div class="caption product-info clearfix">
					...
					<div class="description_options">
						...
						<?php if ($revtheme_product_all['options_buy'] && $product['options'] && $product['options_buy']) { ?>
							<div class="well well-sm product-info options_buy">
								<div id="option_<?php echo $product['product_id']; ?>" class="options">
									<div class="form-group">
										...
										<?php } ?>
										<select class="form-control" onchange="get_product_option_id_slider_m_f('<?php echo $product['product_id']; ?>')" name="" id="slider_m_f_soption_<?php echo $product['product_id']; ?>">
							...
							$('#slider_m_f select.form-control option:nth-child(1)').attr('selected', 'selected');
							$( document ).ready(function() {
								get_product_option_id_slider_m_f (<?php echo $product['product_id']; ?>);
							});
							function get_product_option_id_slider_m_f (product_id) {
								...
								update_prices_product_slider_m_f (product_id,1);
							}
							--></script>
								...
								<?php foreach ($product['options'] as $option) { ?>
									<?php if ($option['type'] == 'select') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
											<label class="control-label" for="slider_m_f_input-option<?php echo $option['product_option_id']; ?>"><?php echo $option['name']; ?>:</label>
											<select onchange="update_prices_product_slider_m_f(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" name="option[<?php echo $option['product_option_id']; ?>]" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control">
											...
											<div id="input-option<?php echo $option['product_option_id']; ?>">
												<?php foreach ($option['product_option_value'] as $option_value) { ?>
													<div class="radio<?php if ($option['opt_image']) { ?> opt_with_img<?php } ?>">
														<input onchange="update_prices_product_slider_m_f(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="slider_m_f_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
														<label <?php if ($option['opt_image']) { ?>data-toggle="tooltip" title="<?php echo $option_value['name']; ?>"<?php } ?> class="btn btn-default <?php if ($option_value['image']) { ?>label-in-img<?php } ?>" for="slider_m_f_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>">
									...
									<?php if ($option['type'] == 'text') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
										  <label class="control-label" for="slider_m_f_input-option<?php echo $option['product_option_id']; ?>"><?php echo $option['name']; ?>:</label>
										  <input type="text" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['value']; ?>" placeholder="<?php echo $option['name']; ?>" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control" />
										</div>
									<?php } ?>
									<?php if ($option['type'] == 'textarea') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
										  <label class="control-label" for="slider_m_f_input-option<?php echo $option['product_option_id']; ?>"><?php echo $option['name']; ?>:</label>
										  <textarea name="option[<?php echo $option['product_option_id']; ?>]" rows="5" placeholder="<?php echo $option['name']; ?>" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control"><?php echo $option['value']; ?></textarea>
										</div>
									<?php } ?>
									...
									<?php if ($option['type'] == 'date') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
										  ...
										  <div class="input-group date">
											<input type="text" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['value']; ?>" data-date-format="YYYY-MM-DD" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control" />
									...
									<?php if ($option['type'] == 'datetime') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
										  ...
										  <div class="input-group datetime">
											<input type="text" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['value']; ?>" data-date-format="YYYY-MM-DD HH:mm" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control" />
											<span class="input-group-btn">
									...
									<?php if ($option['type'] == 'time') { ?>
										<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
										  ...
										  <div class="input-group time">
											<input type="text" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['value']; ?>" data-date-format="HH:mm" id="slider_m_f_input-option<?php echo $option['product_option_id']; ?>" class="form-control" />
											<span class="input-group-btn">
											<button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button>
											</span></div>
										</div>
					...
					<div class="product_buttons">
						...
						<?php if ($product['quantity'] > 0 || $zakaz || $predzakaz_button) { ?>
							<?php if ($product['price_number'] > 0 || $zakaz_price_null) { ?>
								...
								<?php } else { ?>
									<div class="cart">
										<a onclick="get_revpopup_cart(<?php echo $product['product_id']; ?>,'module',get_revpopup_cart_quantity_slider_m_f('<?php echo $product['product_id']; ?>'),'slider_m_f');" <?php if ($button_cart_class != 'prlistb active') { ?>data-toggle="tooltip" title="<?php echo $button_cart; ?>"<?php } ?>><i class="fa fa-border fa-shopping-basket"><span class="<?php echo $button_cart_class; ?>"><?php echo $button_cart; ?></span></i></a>
									</div>
...
<?php if ($img_slider) { ?>
	$('#slider_m_f .owlproduct').owlCarousel({
		items: 1,
		singleItem: true,
		mouseDrag: false,
		touchDrag: false,
...
<?php foreach ($products as $product) { if ($product['minimum'] > 1 && $recalc_price) { ?>
	update_quantity_slider_m_f(<?php echo $product['product_id']; ?>, <?php echo $product['minimum']; ?>);
<?php } } ?>
		...
		<?php if ($recalc_price) { ?>
			...
			<?php if ($description_options['zamena_description'] && $description_options['weight']) { ?>
				...
				<?php if ($revtheme_product_all['recalc_price_animate']) { ?>
					var start_weight = parseFloat($('#slider_m_f .pr_weight_'+product_id).attr('data-weight'));
					$({val:start_weight}).animate({val:weight}, {
						duration: 500,
						easing: 'swing',
						step: function(val) {
							$('#slider_m_f .pr_weight_'+product_id).html(weight_format(val, product_id));
						}
					});
					$('#slider_m_f .pr_weight_'+product_id).attr('data-weight', json['weight']);
				<?php } else { ?>
					$('#slider_m_f .pr_weight_'+product_id).html(weight_format(weight, product_id));
				<?php } ?>
			<?php } ?>
			...
			<?php if ($stikers_status) { ?>
				...
				<?php } else { ?>
					$('#slider_m_f .special_no_format'+product_id).html(price_format(price));
				<?php } ?>
			<?php } ?>
			...
			<?php } else { ?>
				$('#slider_m_f .price_no_format'+product_id).html(price_format(special));
			<?php } ?>
		<?php } ?>
	  }
	});
...
function update_prices_product_slider_m_f(product_id, minimumvalue) {
	...
	$.ajax({
	  ...
	  success: function(json) {
		...
		<?php if ($img_slider) { ?>
		$('#slider_m_f .product_'+product_id+' .image .owl-item:first-child img').attr('src', json['opt_image']);
		<?php } else { ?>
		...
		$('#slider_m_f .product_'+product_id+' .pr_model_'+product_id).html(json['opt_model']);
		<?php } ?>
		...
		<?php } else { ?>
			$('#slider_m_f .pr_quantity_'+product_id).html(number_format(end_quantity, product_id));
		<?php } ?>
...
max_height_div('#slider_m_f .product-thumb .product_buttons');
