<?php
class ControllerRevolutionAccountRevEdit extends Controller {
	private $error = array();

	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('revolution/account/revedit', '', true);
		...
		$this->document->setTitle($this->language->get('text_rev_text_edit'));
		$data['text_none'] = $this->language->get('text_revacc_none');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->session->data['success'] = $this->language->get('text_revaccount_save_success');
			...
			$this->response->redirect($this->url->link('revolution/account/revaccount', '', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('revolution/account/revaccount', '', true)
		);

		$data['breadcrumbs'][] = array(
			'text'      => $this->language->get('text_edit'),
			'href'      => $this->url->link('revolution/account/revedit', '', true)
		);

		$data['heading_title'] = $this->language->get('text_rev_text_edit');

		$data['text_your_details'] = $this->language->get('text_your_details');
		$data['text_additional'] = $this->language->get('text_additional');
		$data['text_select'] = $this->language->get('text_select');
		$data['text_loading'] = $this->language->get('text_loading');
		$data['text_rev_on'] = $this->language->get('text_rev_on');
		...
		$data['entry_password'] = $this->language->get('entry_rev_password');
		$data['entry_rev_newsletter'] = $this->language->get('entry_rev_newsletter');
		...
		$data['button_continue'] = $this->language->get('button_revaccount_save');
		...
		$data['action'] = $this->url->link('revolution/account/revedit', '', true);
		...
		}  elseif (!empty($address_info) && $address_info['country_id'] != '' && $address_info['country_id'] != 0) {
		...
		$data['custom_fields_addr'] = $this->model_account_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
		if (isset($this->request->post['custom_field'])) {
		...
		$data['back'] = $this->url->link('revolution/account/revaccount', '', true);
		...
		if (VERSION >= 2.2) {
			$this->response->setOutput($this->load->view('revolution/account/revedit', $data));
		} else {
			$this->response->setOutput($this->load->view('revolution/account/revedit.tpl', $data));
		}
	}
	...
	protected function validate() {
		...
		if ($settings['revregister_mail'] == '2' && ($this->customer->getEmail() != $this->request->post['email']) && $this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])) {
