...
'opt_image' => $opt_image, 'model' => $option_value_query->row['model'],
							...
							foreach ($value as $product_option_value_id) {
								$option_value_query = $this->db->query("SELECT pov.option_value_id, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix, pov.opt_image, pov.model, ovd.name FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (pov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$product_option_value_id . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
...
'opt_image' => $opt_image, 'model' => $option_value_query->row['model'],
				...
				$product_data[] = array(
					...
					'model'           => $product_query->row['model'], 'sku' => $product_query->row['sku'],
					...
					'stock'           => $stock,
					'price'           => $allprice,
					'total'           => $allprice * $cart['quantity'],
					'reward'          => $reward * $cart['quantity'],
					'points'          => ($product_query->row['points'] ? ($product_query->row['points'] + $option_points) * $cart['quantity'] : 0),
					'tax_class_id'    => $product_query->row['tax_class_id'],
					'weight'          => $option_weight * $cart['quantity'],
