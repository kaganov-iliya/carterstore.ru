...
class ControllerStartupSeoUrl extends Controller {
		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				...
				if ($url[0] == 'revblog_category_id') {
					...
					if ($query->row['query'] && $url[0] != 'revblog_category_id' && $url[0] != 'revblog_id' && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'revolution/revblog_blog' && $key == 'revblog_id') || ($data['route'] == 'information/information' && $key == 'information_id')) {
