		...
		if ($order_info) {
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach($product_query->rows as $product) {
					...
					$option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option oo LEFT JOIN " . DB_PREFIX . "product_option_value pov ON oo.product_option_id = pov.product_option_id WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "' GROUP BY oo.order_id");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($product['image']) {
					...
					$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option oo LEFT JOIN " . DB_PREFIX . "product_option_value pov ON oo.product_option_id = pov.product_option_id WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "' GROUP BY oo.order_id");
...
'model' => (isset($option['model']) ? $option['model'] : false),
					...
					$data['products'][] = array(
						'name'     => $product['name'],
						'model'    => $product['model'], 'sku' => $product['sku'], 'image' => $image, 'href' => $this->url->link('product/product', 'product_id=' . $product['product_id']),
						'option'   => $option_data,
